...
class Array(object):
   ...
   def oddEvenSort(self):  #Ordenamiento con pares e impares
      n = len(self.__a)    #Longitud de la lista
      sorted = False       #Verifica si la lista esta ordenada
      while not sorted:
         sorted = True
         #Examina posiciones impares, haciendo saltos de 2 en 2
         for i in range(0, n-1, 2):
               if self.__a[i] > self.__a[i+1]:
                  self.__a[i], self.__a[i+1] = self.__a[i+1], self.__a[i]
                  sorted = False #La lista aun no esta ordenada porque se hizo un intercambo
         #Examina posiciones pares, haciendo saltos de 2 en 2
